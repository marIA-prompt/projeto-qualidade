...
| `supabase_schema_rls.sql` | Schema Postgres completo com RLS (rodar no SQL Editor do Supabase) |
| `.env.example` | Modelo de variáveis de ambiente |

## Como subir do zero

1. **Banco**: crie um projeto no Supabase (plano gratuito) e execute
   `supabase_schema_rls.sql` inteiro no SQL Editor.
2. **Usuários**: crie os usuários em *Authentication → Users* e insira o perfil
   de cada um na tabela `perfis` (`staff` ou `correspondente`) — instruções no
   final do próprio SQL.
3. **Ambiente**: `cp .env.example .env` e preencha `SUPABASE_URL`,
   `SUPABASE_ANON_KEY` e `SUPABASE_SERVICE_ROLE_KEY`.
4. **Dependências**: `pip install -r requirements.txt`
5. **ETL do mês** (usa a service_role key, roda fora de contexto de usuário):
...
6. **Denominador do índice**: carregue a volumetria de carteira produzida
   (acumulada desde jan/2023) na tabela `carteira_produzida` — entrada manual
   até a fonte oficial ser confirmada. Sem ela, o correspondente fica como
   `nao_aplicavel` (nunca é assumido zero). Depois rode o ETL de novo.
7. **Painel**: `streamlit run dashboard/app.py` (usa apenas a anon key; o RLS
